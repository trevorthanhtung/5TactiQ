  ...
  return (
    <div className="p-4 flex flex-col min-h-full max-w-5xl mx-auto w-full pb-8 animate-pulse">
      ...
      <div className="flex items-center gap-2 @sm:gap-3 mb-6 pt-2">
        ...
        <div>
          <div className="h-9 w-48 bg-text-muted/25 rounded mb-1"></div>
      ...
      <div className="grid grid-cols-2 @md:grid-cols-4 gap-2 @md:gap-4 mb-6">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="hallmark-card bg-surface p-4 flex flex-col justify-center items-center h-24 border-2 border-border-main">
            <div className="h-6 w-6 bg-text-muted/20 rounded-full mb-2"></div>
            <div className="h-6 w-12 bg-text-muted/25 rounded mb-1"></div>
      ...
      <div className="mb-6 flex flex-col @md:flex-row gap-3 items-stretch @md:items-center justify-between">
        <div className="h-[44px] w-full @md:w-80 bg-text-muted/15 border-2 border-border-main rounded"></div>
        <div className="h-[44px] w-full @md:w-[160px] bg-text-muted/20 rounded shrink-0"></div>
      </div>

      {/* List Skeleton */}
      <div className="space-y-3">
        {[1, 2, 3].map((i) => (
          <div key={i} className="hallmark-card bg-surface p-4 flex items-center justify-between border-2 border-border-main h-20">
            <div>
              <div className="h-5 w-32 bg-text-muted/25 rounded mb-2"></div>
              <div className="h-3 w-40 bg-text-muted/15 rounded"></div>
            </div>
            <div className="flex gap-2">
              <div className="h-6 w-6 bg-text-muted/20 rounded"></div>
              <div className="h-6 w-6 bg-text-muted/20 rounded"></div>
            </div>
          </div>
        ))}
